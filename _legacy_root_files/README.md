...
- **Lip-Sync Video** — (Optional) Leaders talk with realistic lip movements using Replicate (SadTalker model).
...
| `REPLICATE_API_TOKEN` | No | Replicate token for lip-sync video ([replicate.com](https://replicate.com/account/api-tokens)) |
...
### With All Optional APIs (PowerShell)

```powershell
$env:GOOGLE_API_KEY="your-google-api-key"; $env:ELEVENLABS_API_KEY="your-elevenlabs-key"; $env:REPLICATE_API_TOKEN="r8_xxx"; streamlit run app.py
```
...
### Lip-Sync Video (Replicate)
If you provide `REPLICATE_API_TOKEN`, the app will automatically generate a talking-head video for each leader response.

- **Engine:** Replicate (SadTalker model)
- **Cost:** ~$0.02-0.05 per video generation (pay-as-you-go).
- **Latency:** Adds ~5-15 seconds to the response time.
- **Setup:** Get your token at [replicate.com/account/api-tokens](https://replicate.com/account/api-tokens). Add `REPLICATE_API_TOKEN` to your secrets. Video generation happens automatically if the key is present.
